...
#include <segway_gazebo_plugins/segway_ros_force_based_move.h>
...
namespace gazebo
{
  ...
  void GazeboRosForceBasedMove::publishOdometry(double step_time)
  {
    ...
    odom_transform_= odom_transform_ * this->getTransformForMotion(
        linear_vel.x,
        linear_vel.y,
        angular_vel.z,
        step_time);
  ...
  tf::Transform GazeboRosForceBasedMove::getTransformForMotion(
    double linear_vel_x,
    double linear_vel_y,
    double angular_vel,
    double timeSeconds) const
  {
    ...
    // Assumes that the timestep is small enough that the robot moves in an
    // approximately straight line.
    tmp.setOrigin(tf::Vector3(linear_vel_x*timeSeconds, linear_vel_y*timeSeconds, 0.0));
    tmp.setRotation(tf::createQuaternionFromYaw(angular_vel*timeSeconds));
